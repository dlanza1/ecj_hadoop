...
public class SimpleBreeder extends Breeder {
	...
	public boolean usingElitism(int subpopulation) {
		return (elite[subpopulation] != NOT_SET)
				|| (eliteFrac[subpopulation] != NOT_SET);
	}
	...
	public int numElites(EvolutionState state, int subpopulation) {
		if (elite[subpopulation] != NOT_SET) {
			...
			return (int) Math
					.max(Math
							.floor(state.population.subpops[subpopulation].individuals.length
									* eliteFrac[subpopulation]), 1.0); // AT
																		// LEAST
																		// 1
																		// ELITE
		} else {
			state.output
					.warnOnce("Elitism error (SimpleBreeder).  This shouldn't be able to happen.  Please report.");
	...
	public void setup(final EvolutionState state, final Parameter base) {
		...
		sequentialBreeding = state.parameters.getBoolean(
				base.push(P_SEQUENTIAL_BREEDING), null, false);
		if (sequentialBreeding && (size == 1)) // uh oh, this can't be right
			state.output
					.fatal("The Breeder is breeding sequentially, but you have only one population.",
							base.push(P_SEQUENTIAL_BREEDING));

		clonePipelineAndPopulation = state.parameters.getBoolean(
				base.push(P_CLONE_PIPELINE_AND_POPULATION), null, true);
		if (!clonePipelineAndPopulation && (state.breedthreads > 1)) // uh oh,
																		// this
																		// can't
																		// be
																		// right
			state.output
					.fatal("The Breeder is not cloning its pipeline and population, but you have more than one thread.",
							base.push(P_CLONE_PIPELINE_AND_POPULATION));

		int defaultSubpop = state.parameters.getInt(new Parameter(
				Initializer.P_POP).push(Population.P_DEFAULT_SUBPOP), null, 0);
		for (int x = 0; x < size; x++) {
			// get elites
			if (state.parameters.exists(base.push(P_ELITE).push("" + x), null)) {
				if (state.parameters.exists(base.push(P_ELITE_FRAC)
						.push("" + x), null))
					state.output.error(
							"Both elite and elite-frac specified for subpouplation "
									+ x + ".",
							base.push(P_ELITE_FRAC).push("" + x),
							base.push(P_ELITE_FRAC).push("" + x));
				else {
					elite[x] = state.parameters.getIntWithDefault(
							base.push(P_ELITE).push("" + x), null, 0);
					if (elite[x] < 0)
						state.output.error("Elites for subpopulation " + x
								+ " must be an integer >= 0", base
								.push(P_ELITE).push("" + x));
				}
			} else if (state.parameters.exists(
					base.push(P_ELITE_FRAC).push("" + x), null)) {
				eliteFrac[x] = state.parameters.getDoubleWithMax(
						base.push(P_ELITE_FRAC).push("" + x), null, 0.0, 1.0);
				if (eliteFrac[x] < 0.0)
					state.output
							.error("Elite Fraction of subpopulation "
									+ x
									+ " must be a real value between 0.0 and 1.0 inclusive",
									base.push(P_ELITE_FRAC).push("" + x));
			} else if (defaultSubpop >= 0) {
				if (state.parameters.exists(
						base.push(P_ELITE).push("" + defaultSubpop), null)) {
					elite[x] = state.parameters.getIntWithDefault(
							base.push(P_ELITE).push("" + defaultSubpop), null,
							0);
					if (elite[x] < 0)
						state.output
								.warning("Invalid default subpopulation elite value."); // we'll
																						// fail
																						// later
				} else if (state.parameters.exists(base.push(P_ELITE_FRAC)
						.push("" + defaultSubpop), null)) {
					eliteFrac[x] = state.parameters.getDoubleWithMax(
							base.push(P_ELITE_FRAC).push("" + defaultSubpop),
							null, 0.0, 1.0);
					if (eliteFrac[x] < 0.0)
						state.output
								.warning("Invalid default subpopulation elite-frac value."); // we'll
																								// fail
																								// later
				} else // elitism is 0
				{
			...
			// get reevaluation
			if (defaultSubpop >= 0
					&& !state.parameters.exists(base.push(P_REEVALUATE_ELITES)
							.push("" + x), null)) {
				reevaluateElites[x] = state.parameters
						.getBoolean(
								base.push(P_REEVALUATE_ELITES).push(
										"" + defaultSubpop), null, false);
				if (reevaluateElites[x])
					state.output
							.warning("Elite reevaluation not specified for subpopulation "
									+ x
									+ ".  Using values for default subpopulation "
									+ defaultSubpop
									+ ": "
									+ reevaluateElites[x]);
			} else {
				reevaluateElites[x] = state.parameters.getBoolean(
						base.push(P_REEVALUATE_ELITES).push("" + x), null,
						false);
			}
		}
	...
	public int computeSubpopulationLength(EvolutionState state,
			Population newpop, int subpopulation, int threadnum) {
		if (!shouldBreedSubpop(state, subpopulation, threadnum))
			return newpop.subpops[subpopulation].individuals.length; // we're
																		// not
																		// breeding
																		// the
																		// population,
		...
		return newpop.subpops[subpopulation].individuals.length
				- numElites(state, subpopulation); // we're breeding population,
													// so elitism may have
													// happened
	}
	...
	public Population breedPopulation(EvolutionState state) {
		...
		// how many threads do we really need? No more than the maximum number
		// of individuals in any subpopulation
		int numThreads = 0;
		for (int x = 0; x < state.population.subpops.length; x++)
			numThreads = Math.max(numThreads,
		...
		if (numThreads < state.breedthreads)
			state.output.warnOnce("Largest subpopulation size (" + numThreads
					+ ") is smaller than number of breedthreads ("
					+ state.breedthreads
		...
		for (int x = 0; x < state.population.subpops.length; x++) {
			...
			for (int y = 0; y < numThreads; y++) {
				...
				if (numinds[y][x] == 0) {
					state.output
							.warnOnce("More threads exist than can be used to breed some subpopulations (first example: subpopulation "
									+ x + ")");
				}
		...
		/*
		 * for(int y=0;y<state.breedthreads;y++) for(int
		 * x=0;x<state.population.subpops.length;x++) { // the number of
		 * individuals we need to breed int length =
		 * computeSubpopulationLength(state, newpop, x, 0); // the size of each
		 * breeding chunk except the last one int firstBreedChunkSizes =
		 * length/state.breedthreads; // the size of the last breeding chunk int
		 * lastBreedChunkSize = firstBreedChunkSizes + length -
		 * firstBreedChunkSizes * (state.breedthreads);
		 * 
		 * // figure numinds if (y < state.breedthreads-1) // not the last one
		 * numinds[y][x] = firstBreedChunkSizes; else // the last one
		 * numinds[y][x] = lastBreedChunkSize;
		 * 
		 * // figure from from[y][x] = (firstBreedChunkSizes * y); }
		 */
		if (numThreads == 1) {
			...
			for (int y = 0; y < numThreads; y++)
				try {
					...
					state.output
							.fatal("Whoa! The main breeding thread got interrupted!  Dying...");
				}
		}
		return newpop;
	}
	...
	/**
	 * Returns true if we're doing sequential breeding and it's the
	 * subpopulation's turn (round robin, one subpopulation per generation).
	 */
	public boolean shouldBreedSubpop(EvolutionState state, int subpop,
			int threadnum) {
	...
	protected void loadElites(EvolutionState state, Population newpop) {
			...
			if (numElites(state, x) > state.population.subpops[x].individuals.length)
				state.output.error("The number of elites for subpopulation "
						...
						new Parameter(EvolutionState.P_BREEDER).push(P_ELITE)
								.push("" + x));
			if (numElites(state, x) == state.population.subpops[x].individuals.length)
				state.output.warning("The number of elites for subpopulation "
						+ x + " is the actual size of the subpopulation",
						new Parameter(EvolutionState.P_BREEDER).push(P_ELITE)
								.push("" + x));
		}
		state.output.exitIfErrors();
		...
		for (int sub = 0; sub < state.population.subpops.length; sub++) {
			if (!shouldBreedSubpop(state, sub, 0)) // don't load the elites for
			...
			} else if (numElites(state, sub) > 0) // we'll need to sort
			{
				...
				QuickSort.qsort(orderedPop, new EliteComparator(
