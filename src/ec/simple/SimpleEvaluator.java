...
public class SimpleEvaluator extends Evaluator {
	...
	public void evaluatePopulation(final EvolutionState state) {
		...
		if (state.evalthreads == 1) {
			...
			for (int i = 0; i < threads.length; i++) {
				...
				threads[i] = pool
						.startThread("ECJ Evaluation Thread " + i, run);
			}
	...
	protected void evalPopChunk(EvolutionState state, int[] numinds,
			int[] from, int threadnum, SimpleProblemForm p) {
	...
	class SimpleEvaluatorThreadCG implements Runnable {
		...
		public void run() {
			...
			while (true) {
				// We need to grab the information about the next chunk we're
				// responsible for. This stays longer
				...
				synchronized (lock) {
					...
					if (individualCounter >= subpops[subPopCounter].individuals.length) // try
																						// again,
																						// jump
																						// to
																						// next
					...
					count = chunkSize;
					if (count == C_AUTO) // compute automatically for
											// subpopulations
						count = computeChunkSizeForSubpopulation(state, subpop,
								threadnum);

					individualCounter += count; // it can be way more than we'll
												// actually do, that's fine
				}
