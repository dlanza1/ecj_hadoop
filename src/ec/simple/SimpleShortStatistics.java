 ...
 * </tr>
 * <tr>
 * <td valign=top><i>base.</i><tt>modulus</tt><br>
 * <font size=-1>integer >= 1 (default)</font></td>
 * <td valign=top>(How often (in generations) should we print a statistics
 * line?)</td>
 * </tr>
 * <tr>
 * <td valign=top><i>base</i>.<tt>do-time</tt><br>
 * <font size=-1>bool = <tt>true</tt> or <tt>false</tt> (default)</font></td>
 * <td valign=top>(print timing information?)</td>
 * </tr>
 * <tr>
 * <td valign=top><i>base</i>.<tt>do-size</tt><br>
 * <font size=-1>bool = <tt>true</tt> or <tt>false</tt> (default)</font></td>
 * <td valign=top>(print sizing information?)</td>
 * </tr>
 * <tr>
 * <td valign=top><i>base</i>.<tt>do-subpops</tt><br>
 * <font size=-1>bool = <tt>true</tt> or <tt>false</tt> (default)</font></td>
...
public class SimpleShortStatistics extends Statistics {
	...
	public long[] totalSizeThisGen; // per-subpop total size of individuals this
									// generation
	public double[] totalFitnessThisGen; // per-subpop mean fitness this
											// generation
	public Individual[] bestOfGeneration; // per-subpop best individual this
											// generation
	...
	public void setup(final EvolutionState state, final Parameter base) {
		...
		modulus = state.parameters.getIntWithDefault(
				base.push(P_STATISTICS_MODULUS), null, 1);
		...
		if (state.parameters.exists(base.push(P_FULL), null)) {
			state.output
					.warning(
							...
							base.push(P_FULL), null);
			boolean gather = state.parameters.getBoolean(base.push(P_FULL),
					null, false);
			doSize = doSize || gather;
			doTime = doTime || gather;
		}
		doSubpops = state.parameters.getBoolean(base.push(P_DO_SUBPOPS), null,
				false);
	}
	...
	public void postBreedingStatistics(final EvolutionState state) {
		...
		if (output)
			state.output
					.print("" + (state.generation + 1) + " ", statisticslog); // 1
																				// because
																				// we're
																				// putting
																				// the
	...
	protected void gatherExtraSubpopStatistics(EvolutionState state,
			int subpop, int individual) {
	}

	protected void printExtraSubpopStatisticsBefore(EvolutionState state,
			int subpop) {
	}

	protected void printExtraSubpopStatisticsAfter(EvolutionState state,
			int subpop) {
	}
	...
	public void postEvaluationStatistics(final EvolutionState state) {
		...
		int subpops = state.population.subpops.length; // number of
														// supopulations
		totalIndsThisGen = new long[subpops]; // total assessed individuals
		bestOfGeneration = new Individual[subpops]; // per-subpop best
													// individual this
													// generation
		totalSizeThisGen = new long[subpops]; // per-subpop total size of
												// individuals this generation
		totalFitnessThisGen = new double[subpops]; // per-subpop mean fitness
													// this generation
		double[] meanFitnessThisGen = new double[subpops]; // per-subpop mean
															// fitness this
															// generation
		...
		for (int x = 0; x < subpops; x++) {
			for (int y = 0; y < state.population.subpops[x].individuals.length; y++) {
																			...
																			// fitness
				{
					...
					if (bestOfGeneration[x] == null
						...
						if (bestSoFar[x] == null
								|| bestOfGeneration[x].fitness
										.betterThan(bestSoFar[x].fitness))
							bestSoFar[x] = (Individual) (bestOfGeneration[x]
									.clone());
					}

					// sum up mean fitness for population
					totalFitnessThisGen[x] += state.population.subpops[x].individuals[y].fitness
							.fitness();
			...
			meanFitnessThisGen[x] = (totalIndsThisGen[x] > 0 ? totalFitnessThisGen[x]
					/ totalIndsThisGen[x]
					: 0);
			...
			if (output && doSize && doSubpops) {
				state.output
						.print(""
								+ (totalIndsThisGen[x] > 0 ? ((double) totalSizeThisGen[x])
										/ totalIndsThisGen[x]
										: 0) + " ", statisticslog);
				state.output.print(""
						+ (totalIndsSoFar[x] > 0 ? ((double) totalSizeSoFar[x])
								/ totalIndsSoFar[x] : 0) + " ", statisticslog);
				state.output.print("" + (double) (bestOfGeneration[x].size())
						+ " ", statisticslog);
				state.output.print("" + (double) (bestSoFar[x].size()) + " ",
						statisticslog);
			}

			// print out fitness information
			if (output && doSubpops) {
				state.output.print("" + meanFitnessThisGen[x] + " ",
						statisticslog);
				state.output.print("" + bestOfGeneration[x].fitness.fitness()
						+ " ", statisticslog);
				state.output.print("" + bestSoFar[x].fitness.fitness() + " ",
						statisticslog);
			}
		...
		for (int x = 0; x < subpops; x++) {
			...
			if (bestOfGeneration[x] != null
					&& (popBestOfGeneration == null || bestOfGeneration[x].fitness
							.betterThan(popBestOfGeneration.fitness)))
		...
		// build mean
		popMeanFitness = (popTotalInds > 0 ? popTotalFitness / popTotalInds : 0); // average
																					// out
		// hook for KozaShortStatistics etc.
		if (output)
		...
		if (output && doSize) {
			state.output.print(""
					+ (popTotalInds > 0 ? popTotalSize / popTotalInds : 0)
					+ " ", statisticslog); // mean size of pop this gen
			state.output.print(""
					+ (popTotalIndsSoFar > 0 ? popTotalSizeSoFar
							/ popTotalIndsSoFar : 0) + " ", statisticslog); // mean
																			// size
																			// of
																			// pop
																			// so
																			// far
			state.output.print(
					"" + (double) (popBestOfGeneration.size()) + " ",
					statisticslog); // size of best ind of pop this gen
			state.output.print("" + (double) (popBestSoFar.size()) + " ",
					statisticslog); // size of best ind of pop so far
		}

		// print out fitness info
		if (output) {
			state.output.print(
					new DecimalFormat("#0.0000000000000000")
							.format(popMeanFitness)
					// + popMeanFitness
							+ " ", statisticslog); // mean
													// fitness
													// of
			...
			state.output.print(
					""
							+ new DecimalFormat("#0.0000000000000000")
									.format(popBestOfGeneration.fitness
											.fitness())
							// + (double)
							// (popBestOfGeneration.fitness.fitness())
							+ " ", statisticslog); // best fitness of pop this
													// gen
			state.output.print(
					new DecimalFormat("#0.0000000000000000")
							.format(popBestSoFar.fitness.fitness())
					// + (double) (popBestSoFar.fitness.fitness())
							+ " ", statisticslog); // best fitness of pop so far
		}
