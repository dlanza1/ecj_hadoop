...
public class TournamentSelection extends SelectionMethod implements
	...
	public void setup(final EvolutionState state, final Parameter base) {
		...
		double val = state.parameters.getDouble(base.push(P_SIZE),
				def.push(P_SIZE), 1.0);
		if (val < 1.0)
			state.output.fatal("Tournament size must be >= 1.",
					base.push(P_SIZE), def.push(P_SIZE));
