...
public class KozaFitness extends Fitness {
	 ...
	 * Adjusted Fitness.
	 */
	public final void setStandardizedFitness(final EvolutionState state,
			final float _f) {
		if (_f < 0.0f || _f == Float.POSITIVE_INFINITY || Float.isNaN(_f)) {
			state.output
					.warning("Bad fitness (may not be < 0, NaN, or infinity): "
							+ _f + ", setting to 0.");
	...
	public final boolean isIdealFitness() {
		return standardizedFitness <= 0.0f; // should always be == 0.0f, <0.0f
											// is illegal, but just in case...
	}
	...
	public String fitnessToString() {
		return FITNESS_PREAMBLE + Code.encode(standardizedFitness)
				+ Code.encode(hits);
	}

	public String fitnessToStringForHumans() {
		return FITNESS_PREAMBLE + "Standardized=" + standardizedFitness
				+ " Adjusted=" + adjustedFitness() + " Hits=" + hits;
	}
