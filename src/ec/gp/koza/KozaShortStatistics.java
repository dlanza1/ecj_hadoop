...
public class KozaShortStatistics extends SimpleShortStatistics {
	...
	public void setup(final EvolutionState state, final Parameter base) {
		...
		doDepth = state.parameters.getBoolean(base.push(P_DO_DEPTH), null,
				false);
	}
	...
	protected void gatherExtraSubpopStatistics(EvolutionState state,
		...
		for (int z = 0; z < i.trees.length; z++) {
			...
			totalSizeThisGenTree[subpop][z] += i.trees[z].child
					.numNodes(GPNode.NODESEARCH_ALL);
	...
	protected void printExtraSubpopStatisticsBefore(EvolutionState state,
		...
		if (doDepth) {
			...
			for (int z = 0; z < totalDepthThisGenTree[subpop].length; z++)
				state.output
						.print(""
								+ (totalIndsThisGen[subpop] > 0 ? ((double) totalDepthThisGenTree[subpop][z])
										/ totalIndsThisGen[subpop]
										: 0) + " ", statisticslog);
			state.output.print("] ", statisticslog);
		}
		if (doSize) {
			...
			for (int z = 0; z < totalSizeThisGenTree[subpop].length; z++)
				state.output
						.print(""
								+ (totalIndsThisGen[subpop] > 0 ? ((double) totalSizeThisGenTree[subpop][z])
										/ totalIndsThisGen[subpop]
										: 0) + " ", statisticslog);
			state.output.print("] ", statisticslog);
		}
...
//			state.output.print("[ ", statisticslog);
			for (int z = 0; z < totalDepthThisGenTreePop.length; z++)
				state.output
						.print(""
								+ (totalIndsThisGenPop > 0 ? ((double) totalDepthThisGenTreePop[z])
										/ totalIndsThisGenPop
										: 0) + " ", statisticslog);
//			state.output.print("] ", statisticslog);
		}
...
//			state.output.print("[ ", statisticslog);
			for (int z = 0; z < totalSizeThisGenTreePop.length; z++)
				state.output
						.print(""
								+ (totalIndsThisGenPop > 0 ? ((double) totalSizeThisGenTreePop[z])
										/ totalIndsThisGenPop
										: 0) + " ", statisticslog);
//			state.output.print("] ", statisticslog);
		}
