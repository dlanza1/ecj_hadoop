...
public class GPIndividual extends Individual {
	...
	public void setup(final EvolutionState state, final Parameter base) {
		...
		// how many trees?
		int t = state.parameters.getInt(base.push(P_NUMTREES),
				def.push(P_NUMTREES), 1); // at least 1 tree for GP!
		if (t <= 0)
		...
		for (int x = 0; x < t; x++) {
			...
			trees[x] = (GPTree) (state.parameters.getInstanceForParameterEq(p,
					def.push(P_TREE).push("" + x), GPTree.class));
			trees[x].owner = this;
